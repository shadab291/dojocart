...
import { loadProducts, STATUSES } from '../Store/productSlice'; // Renamed here
...
const Products = () => {
  ...
  useEffect(() => {
    dispatch(loadProducts()); // Renamed function call here
  }, [dispatch]);
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 p-4 bg-whitesmoke">
      {products.map((product) => (
        <div className="bg-white p-4 rounded-lg shadow-md flex flex-col" key={product.id}>
          ...
          <button onClick={() => dispatch(add(product))} className="mt-auto hover:bg-purple-700 hover:text-white font-semibold py-1 px-2 pr-2 w-full rounded-lg bg-blue-500 text-white">
            Add to Cart
          </button>
        </div>
      ))}
